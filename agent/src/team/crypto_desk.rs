...
const CRYPTO_DESK_SYSTEM: &str = r#"You are a CRYPTO SPECIALIST on a prediction market trading team. You have deep expertise in cryptocurrency markets, on-chain data, macro trends, and crypto-specific patterns.
...
CRYPTO EXPERTISE:
- Analyze price targets using historical BTC/ETH/SOL cycles and support/resistance levels
- RSI >70 is overbought, <30 oversold — but crypto can stay extreme longer than expected
- Fear/Greed index extremes (>80 or <20) often precede reversals
- BTC dominance shifts signal alt season vs BTC season
- On-chain activity, exchange flows, and whale movements matter
- Halving cycles, ETF flows, regulatory news are key catalysts
- Be specific: cite prices, percentages, dates from the data provided
- confidence_in_data: how reliable is the crypto data for this specific prediction (0.0-1.0)
- Do NOT wrap in markdown code blocks."#;

/// Agent 4: Crypto Desk — Cryptocurrency specialist (Gemini AI)
pub async fn analyze(
    ...
    let user_msg = format!(
        ...
        - Key factors: {factors}\n\
        \n\
        Provide your specialist crypto analysis.",
    ...
    let (text, cost) = gemini.call(CRYPTO_DESK_SYSTEM, &user_msg, 400).await?;
    info!(
        "CryptoDesk: {} (${:.4})",
    ...
    parse_desk_report(&text, &market.id, DeskType::Crypto)
}
...
fn parse_desk_report(text: &str, market_id: &str, desk: DeskType) -> Result<DeskReport> {
    ...
    let parsed: Resp = serde_json::from_str(&json_str)
        .map_err(|e| anyhow::anyhow!("CryptoDesk JSON parse: {e} | {}", &json_str[..json_str.len().min(200)]))?;
